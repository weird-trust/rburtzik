import { a3 as current_component, a0 as getContext, a4 as spread_attributes, S as pop, a5 as sanitize_props, Q as push, a6 as spread_props, $ as fallback, Z as head, a7 as add_styles, a8 as merge_styles, a2 as stringify, W as attr, Y as bind_props, V as ensure_array_like, a1 as escape_html } from "../../../chunks/index.js";
...
import { constructCloudinaryUrl, getVideoPlayerOptions } from "@cloudinary-util/url-loader";
...
function list(options, fallback2 = options[0]) {
  return validator(fallback2, (input, keypath) => {
...
const __vite_import_meta_env__ = { "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SSR": true, "VITE_CLOUDINARY_API_KEY": "291722791283983", "VITE_CLOUDINARY_API_SECRET": "gt1v-nKFL7K_evJ7kkW2J64dVqQ", "VITE_CLOUDINARY_CLOUD_NAME": "dk2ozewvs", "VITE_CLOUDINARY_UPLOAD_PRESET": "ml_default" };
...
const DEFAULT_ANALYTICS = Object.freeze({
  techVersion: normaliseVersion(VERSION),
  sdkSemver: normaliseVersion(VERSION$1),
  sdkCode: "E",
  product: "B",
  feature: ""
});
function getEnvConfig() {
  return {
    cloud: {
      cloudName: "dk2ozewvs",
      apiKey: "291722791283983"
    },
    url: {
      privateCdn: !!__vite_import_meta_env__?.VITE_CLOUDINARY_PRIVATE_CDN,
      secureDistribution: __vite_import_meta_env__?.VITE_CLOUDINARY_SECURE_DISTRIBUTION
    },
    extra: {
...
function mergeGlobalConfig(configOverride, analyticsOverride) {
  ...
  const extra = defu({ uploadPreset: globalConfig?.uploadPreset }, envConfig.extra);
  return {
    config,
    analytics,
    extra
...
function getGlobalConfig() {
  try {
    const config = getContext(STORE_KEY);
    return config ?? null;
  } catch (error) {
    return null;
  }
}
function getCldImageUrl(options, configOverride, analyticsOverride) {
  const { config, analytics } = mergeGlobalConfig(configOverride, analyticsOverride);
...
function CldImage($$payload, $$props) {
  const $$sanitized_props = sanitize_props($$props);
  push();
  let width, height, layout, props, transformer;
  ({ width, height, layout, ...props } = $$sanitized_props);
  transformer = createLoader($$sanitized_props);
  $$payload.out += `<!---->`;
  {
    Image($$payload, spread_props([
      {
...
function CldVideoPlayer($$payload, $$props) {
  const $$sanitized_props = sanitize_props($$props);
  push();
  let config, id, videoPlayerOptions, options;
  const PLAYER_VERSION = "1.11.1";
  let loaded = typeof window != "undefined" && !!window.cloudinary?.videoPlayer;
  let videoElement = fallback($$props["videoElement"], void 0);
  let player = fallback($$props["player"], void 0);
  onDestroy(() => {
    player?.dispose();
  });
  ({ config, id, ...videoPlayerOptions } = $$sanitized_props);
  options = getVideoPlayerOptions(videoPlayerOptions, mergeGlobalConfig(config).config);
  if (videoElement && loaded && !player) {
    player = window.cloudinary?.videoPlayer?.(videoElement, options);
    player?.on("error", () => videoPlayerOptions.onError?.({ player, video: videoElement }));
    player?.on("loadeddata", () => videoPlayerOptions.onDataLoad?.({ player, video: videoElement }));
    player?.on("loadedmetadata", () => videoPlayerOptions.onMetadataLoad?.({ player, video: videoElement }));
    player?.on("pause", () => videoPlayerOptions.onPause?.({ player, video: videoElement }));
    player?.on("play", () => videoPlayerOptions.onPlay?.({ player, video: videoElement }));
    player?.on("ended", () => videoPlayerOptions.onEnded?.({ player, video: videoElement }));
  }
  head($$payload, ($$payload2) => {
    $$payload2.out += `<link${attr("href", `https://unpkg.com/cloudinary-video-player@${stringify(PLAYER_VERSION)}/dist/cld-video-player.min.css`)} rel="stylesheet">`;
  });
  $$payload.out += `<div${add_styles(merge_styles("width: 100%;", {
    "aspect-ratio": `${stringify(options.width)} / ${stringify(options.height)}`
  }))}><video${attr("id", id || options.publicId)}${attr("width", options.width)}${attr("height", options.height)} class="cld-video-player cld-fluid"><track kind="captions"></video></div>`;
  bind_props($$props, { videoElement, player });
  pop();
}
function ProjectArrows($$payload, $$props) {
  push();
  const ROWS = 4;
  const COLS = 4;
  ...
  $$payload.out += `<main class="svelte-zi0kl1"><!--[-->`;
  for (let projectIndex = 0, $$length = each_array.length; projectIndex < $$length; projectIndex++) {
    let project = each_array[projectIndex];
    const each_array_1 = ensure_array_like(rows);
    $$payload.out += `<section class="project-section svelte-zi0kl1"${attr("data-project-id", project.id)}><div class="arrow-container svelte-zi0kl1"><!--[-->`;
    for (let rowIndex = 0, $$length2 = each_array_1.length; rowIndex < $$length2; rowIndex++) {
      each_array_1[rowIndex];
      const each_array_2 = ensure_array_like(cols);
      $$payload.out += `<div class="row svelte-zi0kl1"><!--[-->`;
      for (let colIndex = 0, $$length3 = each_array_2.length; colIndex < $$length3; colIndex++) {
        let arrow = each_array_2[colIndex];
        $$payload.out += `<span${attr("class", `arrow svelte-zi0kl1 ${stringify([colIndex === COLS - 1 ? "last-column" : ""].filter(Boolean).join(" "))}`)}>${escape_html(arrow)} `;
        if (colIndex !== COLS - 1) {
          $$payload.out += "<!--[-->";
          $$payload.out += `<div class="hover-content svelte-zi0kl1"><span class="project-name">${escape_html(project.name)}</span> <span class="project-type">${escape_html(project.type)}</span> <span class="project-desc">${escape_html(project.year)}</span></div>`;
        } else {
          $$payload.out += "<!--[!-->";
        }
        $$payload.out += `<!--]--></span>`;
    ...
    $$payload.out += `<!--]--></div> <a${attr("href", paraglide_sveltekit_translate_attribute_pass_translateAttribute(`/projects/${project.id}`, void 0))} class="project-title svelte-zi0kl1"><h2 class="svelte-zi0kl1">${escape_html(project.name)}</h2> `;
    if (project.media?.[0]) {
      $$payload.out += "<!--[-->";
      $$payload.out += `<div class="hover-media svelte-zi0kl1" role="img"${attr("style", `transform: translate(-50%, -50%) perspective(1000px) rotateY(${stringify(mouseX * 10)}deg) rotateX(${stringify(-mouseY * 10)}deg)`)}>`;
...
function _page($$payload, $$props) {
  ...
  $$payload.out += `<!----> <a${attr("href", paraglide_sveltekit_translate_attribute_pass_translateAttribute(`/`, void 0))} class="nav-link home svelte-sht5ek">Robert Burtzik</a> <a${attr("href", paraglide_sveltekit_translate_attribute_pass_translateAttribute(`/`, void 0))} class="nav-link close svelte-sht5ek">Close Projects</a> <a${attr("href", paraglide_sveltekit_translate_attribute_pass_translateAttribute(`/about`, void 0))} class="nav-link about svelte-sht5ek">About</a> <a${attr("href", paraglide_sveltekit_translate_attribute_pass_translateAttribute(`/photo`, void 0))} class="nav-link photo svelte-sht5ek">Photo</a></main>`;
  pop();
}
export {
  _page as default
